...
function visualize(results) {
    ...
    for (let contract_file_name of Object.keys(result.abi)) {
        ...
        for (let name of Object.keys(contract)) {
            ...
            if (name === "gecko" || name.includes("Scribble")) {
                continue
            }
...
async function build_with_autodetect(project, projectType, compiler_version, daemon, autoStart, setupFile) {
    ...
    if (autoStart) {
        let command = "";
        if (setupFile) {
            command = `gecko evm --builder-artifacts-file ./results.json -t "a" --work-dir ./workdir --setup-file ${setupFile}`;
        } else {
            command = `gecko evm --builder-artifacts-file ./results.json --offchain-config-file ./offchain_config.json -f -i -t "a" --work-dir ./workdir`;
        }
        console.log(`Starting gecko with command: ${command}`);
...
const argv = yargs(hideBin(process.argv))
    ...
    .option('auto-start', {
        alias: 's',
        type: 'boolean',
        description: 'AutoXRPally run gecko after building',
    })
    .option('setup-file', {
        alias: 'f',
        type: 'string',
